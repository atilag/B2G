...
if [ "$1" = "attach" ]; then
   ...
   [ "$B2G_BIN" = "/system/b2g/hard/b2g" ] && $ADB shell stop b2g

   if [ "$($ADB shell "if [ -f /system/b2g/hard/libdmd.so ]; then echo 1; fi")" != "" ]; then
     echo ""
     echo "Using DMD."
     echo ""
     dmd="1"
     ld_preload_extra="/system/b2g/hard/libdmd.so"
  fi

   $ADB shell DMD=$dmd LD_LIBRARY_PATH=/system/b2g/hard LD_PRELOAD=\"$ld_preload_extra /system/b2g/hard/libmozglue.so\" TMPDIR=/data/local/tmp $GDBSERVER_ENV gdbserver --multi :$GDB_PORT $B2G_BIN $@ &
fi
